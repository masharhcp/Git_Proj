...
int KernelSem::wait(Time maxTimeToWait){//vraca 1 ukoliko je nit razblokirana metodom signal() a 0 ako je razblokirana zbog cinjenice da je isteklo vreme cekanja
	int retValue;
	value--;
	if (value<0)
		block(maxTimeToWait);
...
KernelSem::~KernelSem(){//ukoliko se semafor brise pre nego sto su deblokirane niti
