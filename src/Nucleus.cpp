...
void interrupt Nucleus::Timer(...){
	...
	if (!demand_context_change && counter>0) {--counter; }//ako nije zahtevana promena konteksta i ako nit nema pravo da se izvrsava beskonacno
	if (!demand_context_change){
		++clock;
		//cout<<endl<<"Clock: "<<Nucleus::clock<<endl;
	while ((pcbQueue->head!=0) && (pcbQueue->Get_First()!=0) && (pcbQueue->Get_First_TTW()<=clock)) Unblock_Timed();
	asm int 60h;
	tick();
}
